    ...
    for (var j = 0; j < filePaths.length; j++) {
        ...
        for (var i = 0; i < plotType.length; i++) {
            ...
            if (column_type == 'pie') {
                ...
                var val = { "file_path": file_path, "column": column_name }
                var map_final_arr = await asyncPostCall('map_vis', val);
                var labelFreq = await asyncPostCall('get_stats', body);
                var final_arr = map_final_arr.final_arr;
                var centre_lat = map_final_arr.centre_lat;
                var centre_long = map_final_arr.centre_long;
                map.visualized_map(file_path, column_name);
